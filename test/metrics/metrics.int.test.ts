...
import type { MetricRegistry } from '../../src/metrics/types.js'
...
describe('MetricRegistry', () => {
  ...
  describe('Process Metrics', () => {
    it('should register process metrics with the expected types', () => {
      registerProcessMetrics(registry)

      expect(registry.getMetric(AUTO_METRICS.PROCESS_CPU)?.type).toBe('counter')
      expect(registry.getMetric(AUTO_METRICS.PROCESS_MEMORY)?.type).toBe('gauge')
      expect(registry.getMetric(AUTO_METRICS.EVENTLOOP_LAG)?.type).toBe('gauge')
    })

    it('should collect process metrics using incremental CPU counters', () => {
      registerProcessMetrics(registry)

      const cpuUsageSamples = [
        { user: 2_000_000, system: 1_000_000 },
        { user: 5_000_000, system: 1_000_000 },
        { user: 6_000_000, system: 1_000_000 },
      ]
      const spy = vi.spyOn(process, 'cpuUsage')
      let call = 0
      spy.mockImplementation(() => cpuUsageSamples[Math.min(call++, cpuUsageSamples.length - 1)])
      ...
      const cpuMetric = registry.getMetric(AUTO_METRICS.PROCESS_CPU)
      const memoryMetric = registry.getMetric(AUTO_METRICS.PROCESS_MEMORY)
      const lagMetric = registry.getMetric(AUTO_METRICS.EVENTLOOP_LAG)

      expect(cpuMetric?.values.get('')?.value).toBe(4)
      expect(memoryMetric?.type).toBe('gauge')
      expect(lagMetric?.type).toBe('gauge')
    })
  })
})
