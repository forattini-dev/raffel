...
function encodeTcpRawPayload(payload: unknown): Buffer {
  if (payload === undefined || payload === null) {
    return Buffer.alloc(0)
  }
...
function frameTcpMessage(message: Record<string, unknown>): Buffer {
...
function connectTcpSocket(entry: RuntimePlaygroundEntry): Promise<Socket> {
  const port = entry.target.port
  if (port === undefined) {
...
function waitForTcpEnvelope(socket: Socket, timeoutMs = 5000): Promise<Record<string, unknown>> {
  return new Promise((resolve, reject) => {
...
function attachTcpEnvelopeStream(
  socket: Socket,
  handlers: {
...
function attachRawTcpMessageStream(
  socket: Socket,
  framing: {
