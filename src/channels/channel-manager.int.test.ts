...
describe('ChannelManager', () => {
  ...
  describe('Public Channels', () => {
    it('should call authorize for public channels when provided', async () => {
      const authorize = vi.fn().mockResolvedValue(true)
      const manager = createChannelManager({ authorize }, mockSend)
      const ctx = createTestContext()

      const result = await manager.subscribe('socket-1', 'chat-room', ctx)

      expect(result.success).toBe(true)
      expect(authorize).toHaveBeenCalledWith('socket-1', 'chat-room', ctx)
    })

    it('should deny public channels when authorize returns false', async () => {
