...
server
  .procedure('lead.read')
  .authz({
    resource: ({ id }) => ({ type: 'lead', id, tenantId: ctx.principal.tenantId }),
  })
...
| Page | When to read |
|---|---|
| **[Guide](../guides/policies.md)** | First read. Concepts, lifecycle, quickstart, debugging — everything you need to ship. |
